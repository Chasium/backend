...
from apigen.generated.card_template.get import GetTemplatesRequest, GetTemplatesResponse, CardTemplate
...
def get(req: GetTemplatesRequest):
    res = GetTemplatesResponse()
    user = get_user(req.session)
    if user is None:
        res.solve__code(1)
        return res

    template_data_list: List[CardTemplateData] = CardTemplateData.query.order_by(CardTemplateData.id.desc()).paginate(
    ...
    all_templates = CardTemplateData.query.count()
